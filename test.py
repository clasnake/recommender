...
critics = {
'cla': {'forest gump': 8.8, 'god http://www.baidu.com/father': 8.6, 'chunking': 8.0, 'Paris': 9.0, 'chow': 8.5,
        'comedy': 6.0},
...
def cal():
    rec = recommender.ItemBasedRecommender(outputFile='cosine_improved_n150.txt',
                                           similarityMeasure=similarity.sim_cosine)
    print
    rec.similarityMeasure;
    rec.loadTrainingSet()
    rec.loadPredictingSet()
    rec.calculateSimilarItems(10, 'cosine_improved_n150.pkl')
    # rec.loadMovieTag()
...
def test():
    ...
    for p in rec.predictPrefs:
        print
        p[0], p[1], rec.predictRating(p[0], p[1])
...
def t():
    rec = recommender.UserBasedRecommender('ub_n80.txt', similarity.sim_cosine)
    rec.loadTrainingSet()
    rec.loadPredictingSet()
...
def t1():
    print
    evaluation.Evaluation('/u1.test', rec=recommender.UserBasedRecommender('/u1.base')).evalByAccuracy()
...
def tcos():
    ...
    print
    similarity.sim_pearson(critics, 'Cla', 'lee')
...
def t5():
    print
    similarity.sim_cosine(critics, 'cla', 'kuuga')
